...
public class WPIAStar extends CommandBase {
  ...
  public void initialize() {
    config.setStartVelocity(
        Math.hypot(
            driveSystem.getChassisSpeeds().vxMetersPerSecond,
            driveSystem.getChassisSpeeds().vxMetersPerSecond));
    ...
    if (AStarMap.addEdge(new Edge(startPoint, finalPosition), obstacles)) {
      ...
      for (int i = 0; i < AStarMap.getNodeSize(); i++) {
        ...
        // System.out.println(String.format("SUCCESS: %f,%f %f,%f - %s", startPoint.getX(),
        // startPoint.getY(), endNode.getX(), endNode.getY(), test ? "true" : "false"));
      }
